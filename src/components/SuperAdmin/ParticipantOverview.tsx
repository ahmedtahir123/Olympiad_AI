...
export const ParticipantOverview: React.FC = () => {
  ...
  return (
    <div className={`p-8 ${isRTL ? 'rtl text-right' : 'ltr text-left'}`}>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">{t('participantsTitle')}</h1>
        <p className="text-gray-600 mt-2">{t('participantsSubtitle')}</p>
      </div>

      {/* Stats Cards */}
      <div className={`grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 ${isRTL ? 'rtl' : 'ltr'}`}>
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t('totalParticipants')}</p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t('academicParticipants')}</p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t('sportsParticipants')}</p>
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t('withResults')}</p>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        <div className={`flex flex-col lg:flex-row gap-4 items-center justify-between ${isRTL ? 'rtl' : 'ltr'}`}>
          <div className="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <option value="all">{t('filterAllEntities')}</option>
                {entities?.map((entity) => (
              ...
              >
                ...
                <option value="sporting">{t('categorySports')}</option>
              </select>
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className={`w-full ${isRTL ? 'rtl' : 'ltr'}`}>
            <thead className="bg-gray-50">
              <tr>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableParticipant')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableEntity')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableGrade')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableCategory')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableCompetitions')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableResults')}</th>
                <th className="py-4 px-6 font-semibold text-gray-700">{t('tableActions')}</th>
              </tr>
            </thead>
            <tbody>
              {filteredParticipants.map((participant) => (
                <tr key={participant.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="py-4 px-6">
                    <div className={`flex items-center ${isRTL ? 'space-x-reverse space-x-3' : 'space-x-3' } `}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{t('ageLabel')} {participant.age}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6">
                  ...
                  <td className="py-4 px-6">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${participant.category === 'academic'
                  ...
                  <td className="py-4 px-6">
                    <div className="flex flex-wrap gap-1">
                      {participant.eventsRegistered.slice(0, 2).map((_, index) => (
                        <span key={index} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs">
                          {t('tableCompetitions')} {index + 1}
                        </span>
                      ))}
                  ...
                  <td className="py-4 px-6">
                    {participant.results && participant.results.length > 0 ? (
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-900">{participant.results.length} {t('tableResults')}</span>
                      </div>
                    ) : (
                      <span className="text-sm text-gray-500">{t('participantModalNoResults')}</span>
                    )}
                  </td>
                  <td className="py-4 px-6">
                    <button
      ...
      {selectedParticipant && (
        <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 ${isRTL ? 'rtl' : 'ltr'}`}>
          <div className="bg-white rounded-xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className={`flex items-center justify-between mb-6 ${isRTL ? 'rtl' : 'ltr'}`}>
              <div className="flex items-center">
                {/* photo */}
                <div>
                  ...
                  <p className="text-gray-600">{selectedParticipant.grade} • {t('ageLabel')} {selectedParticipant.age}</p>
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${selectedParticipant.category === 'academic' ? 'bg-purple-100 text-purple-800' :
                    'bg-orange-100 text-orange-800'}`}>
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 ${isRTL ? 'rtl' : 'ltr'}`}>
              {/* Registrations */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('participantModalRegistrations')}</h3>
                {selectedParticipant.eventsRegistered.map((_, index) => (
                  <div key={index} className="p-4 bg-gray-50 rounded-lg">
                    <p className="font-medium text-gray-900">{t('tableCompetitions')} {index + 1}</p>
                    <p className="text-sm text-gray-600">{t('participantModalStatusRegistered')}</p>
                  </div>
                ))}
              </div>
              ...
              {/* Results */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('participantModalResults')}</h3>
                {selectedParticipant.results && selectedParticipant.results.length > 0 ? (
                    ...
                    <div key={index} className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg border border-yellow-200">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm font-medium">
                          {result.position === 1 ? t('result1st') :
                            result.position === 2 ? t('result2nd') :
                              result.position === 3 ? t('result3rd') :
                                `${result.position}th`}
                        </span>
                      </div>
                      {result.score && <p className="text-sm text-gray-600">{t('scoreLabel')}: {result.score}/100</p>}
                      {result.time && <p className="text-sm text-gray-600">{t('timeLabel')}: {result.time}</p>}
                      {result.certificate && (
                        <button
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">{t('participantModalNoResults')}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {filteredParticipants.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('noParticipantsFound')}</h3>
