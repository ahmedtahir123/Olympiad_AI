...
export const CompetitionDraws: React.FC = () => {
  ...
  <div className="flex items-center justify-between">
    <h3 className="text-xl font-bold text-gray-900">
      {draw.eventName} - {t(draw.drawType)}
    </h3>
  ...
  <div className="overflow-x-auto">
    <div className={`flex space-x-8 min-w-max p-4 ${isRTL ? 'space-x-reverse space-x-8' : 'space-x-8' }`}>
      {Object.entries(rounds).map(([roundNum, matches]) => (
        <div key={roundNum} className="flex flex-col space-y-4">
          <h4 className="text-center font-semibold text-gray-700 mb-4">
            {roundNum === maxRound.toString() ? t("final") : 
             roundNum === (maxRound - 1).toString() ? t("semiFinal") : 
             `${t("round")} ${roundNum}`}
          </h4>
          {matches.map((match) => (
            <div
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className={`flex items-center justify-between p-2 rounded ${
                    ...
                    <span className="font-medium text-sm">{match.participant1 || t("tbd")}</span>
                    <span className="font-bold">{match.score || '-'}</span>
                  </div>

                  <div className="text-center text-xs text-gray-400">{t("vs")}</div>

                  <div className={`flex items-center justify-between p-2 rounded ${
                    match.winner === match.participant2 ? 'bg-green-100 border-green-300' : 'bg-white'
                  }`}>
                    <span className="font-medium text-sm">{match.participant2 || t("tbd")}</span>
                    <span className="font-bold">{match.score || '-'}</span>
                  </div>
                </div>

                {match.scheduledTime && (
                  <div className={`flex items-center text-xs text-gray-500 ${isRTL ? ' space-x-2 space-x-reverse' : 'space-x-2'}`}>
  ...
  <div className="flex items-center justify-between">
    <div>
      <h1 className="text-2xl font-bold text-gray-900">{t("competitionDraws")}</h1>
      <p className="text-gray-600 mt-1">{t("viewTournamentBrackets")}</p>
    </div>
    <div className="flex items-center space-x-2">
      <Trophy className="w-6 h-6 text-yellow-500" />
      <span className="text-sm text-gray-500">{filteredDraws.length} {t("tournaments")}</span>
    </div>
  </div>
    ...
    <div className="flex flex-col sm:flex-row gap-4">
      ...
      <div className="flex items-center space-x-2">
        ...
        >
          ...
          <option value="academic">{t("academicCompetitions")}</option>
          <option value="sporting">{t("sportingCompetitions")}</option>
        </select>
      </div>
    </div>
  </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              <div className={`flex items-center text-sm text-gray-600 ${isRTL ? 'space-x-2 space-x-reverse' : 'space-x-2'}`}>
                ...
                <span>{t(draw.drawType.replace('_', ' '))}</span>
              </div>

              <div className={`flex items-center text-sm text-gray-600 ${isRTL ? ' space-x-2 space-x-reverse' : 'space-x-2'}`}>
                <Users className="w-4 h-4" />
                <span>{draw.totalParticipants} {t("participants")}</span>
              </div>
              ...
              <div className={`flex items-center justify-between pt-3 border-t ${isRTL ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm text-gray-500">
                  {draw.matches.filter(m => m.status === 'completed').length} / {draw.matches.length} {t("matchesCompleted")}
                </span>
  ...
  {filteredDraws.length === 0 && !loading && (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-gray-900 mb-2">{t("noTournamentsFound")}</h3>
      <p className="text-gray-600">
        {searchTerm || selectedEvent !== 'all'
          ? t("tryAdjustingSearch")
          : t("tournamentDrawsWillAppear")
        }
      </p>
    </div>
  )}
