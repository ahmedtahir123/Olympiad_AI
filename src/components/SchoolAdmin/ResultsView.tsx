...
export const ResultsView: React.FC = () => {
  ...
  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl p-6 text-white">
      <div className={`flex items-center justify-between ${isRTL ? "space-x-reverse" : ""}`}>
        <div>
          <p className="text-yellow-100 text-sm">{t("totalResults")}</p>
    ...
    <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-6 text-white">
      <div className={`flex items-center justify-between ${isRTL ? "space-x-reverse" : ""}`}>
        <div>
          <p className="text-green-100 text-sm">{t("medalsWon")}</p>
    ...
    <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 text-white">
      <div className={`flex items-center justify-between ${isRTL ? "space-x-reverse" : ""}`}>
        <div>
          <p className="text-blue-100 text-sm">{t("certificates")}</p>
          <p className="text-3xl font-bold">
            {completedResults.filter(r => r.certificate).length}
          </p>
        </div>
    ...
    <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-6 text-white">
      <div className={`flex items-center justify-between ${isRTL ? "space-x-reverse" : ""}`}>
        <div>
          <p className="text-purple-100 text-sm">{t("pending")}</p>
  ...
  <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
    <div className={`flex flex-col lg:flex-row gap-4 ${isRTL ? "lg:space-x-reverse" : ""}`}>
      ...
      <div className={`flex items-center gap-4 ${isRTL ? "space-x-reverse" : ""}`}>
        ...
        >
          ...
          <option value="Mathematics Competition">{t("mathCompetition")}</option>
          <option value="100m Sprint">{t("sprint100m")}</option>
          <option value="Science Fair">{t("scienceFair")}</option>
          <option value="Basketball Championship">{t("basketball")}</option>
          <option value="Debate Championship">{t("debate")}</option>
        </select>
  ...
  {filteredResults.map((result, index) => {
    ...
    return (
      ...
      >
        {/* Header */}
        <div className={`flex items-start justify-between mb-4 ${isRTL ? "space-x-reverse" : ""}`}>
          <div className={`flex items-center ${isRTL ? "space-x-reverse text-right" : ""}`}>
            <img
        ...
        {result.status === "completed" && positionInfo && (
          <div className="mb-4">
            <div
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg ${positionInfo.color}
        ...
        <div className="space-y-2 mb-4">
          {result.score && (
            <div className={`flex justify-between ${isRTL ? "space-x-reverse" : ""}`}>
        ...
        {/* Actions */}
        {result.status === "completed" && (
          <div className={`flex gap-3 ${isRTL ? "space-x-reverse" : ""}`}>
    ...
    <div className="mt-12 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-8 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="text-center">
          ...
          <p className="text-3xl font-bold">
            {completedResults.filter(r => r.position === 1).length}
          </p>
          <p className="text-blue-100">{t("goldMedals")}</p>
        </div>

        <div className="text-center">
          <div className="text-4xl mb-2">🥈</div>
          <p className="text-3xl font-bold">
            {completedResults.filter(r => r.position === 2).length}
          </p>
          <p className="text-blue-100">{t("silverMedals")}</p>
        </div>

        <div className="text-center">
          <div className="text-4xl mb-2">🥉</div>
          <p className="text-3xl font-bold">
            {completedResults.filter(r => r.position === 3).length}
          </p>
          <p className="text-blue-100">{t("bronzeMedals")}</p>
        </div>
      </div>
    </div>
  )}
